...
nano::json_handler::json_handler(nano::node &node_a, nano::node_rpc_config const &node_rpc_config_a, std::string const &body_a, std::function<void(std::string const &)> const &response_a, std::function<void()> stop_callback_a) : body(body_a),
																																																									 node(node_a),
																																																									 response(response_a),
...
void nano::json_handler::available_supply()
{
	...
	//[Xxxxxxxxxxxxxxxxx_ChangeMe_Xxxxxxxxxxxxxxxxxxx] changed from [059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5]
	auto landing_balance(node.balance(nano::account("Xxxxxxxxxxxxxxxxx_ChangeMe_Xxxxxxxxxxxxxxxxxxx"))); // Active unavailable account
	//[Xxxxxxxxxxxxxxxxx_ChangeMe_Xxxxxxxxxxxxxxxxxxx] changed from [8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B]
	auto faucet_balance(node.balance(nano::account("Xxxxxxxxxxxxxxxxx_ChangeMe_Xxxxxxxxxxxxxxxxxxx")));  // Faucet account
...
namespace
{
	...
	{
	public:
		history_visitor(nano::json_handler &handler_a, bool raw_a, nano::transaction &transaction_a, boost::property_tree::ptree &tree_a, nano::block_hash const &hash_a, std::vector<nano::public_key> const &accounts_filter_a) : handler(handler_a),
																																																									raw(raw_a),
