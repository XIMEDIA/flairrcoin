...
TEST (ledger, receive_rollback)
{
	...
	auto transaction (store.tx_begin (true));
	store.initialize (transaction, genesis);
	nano::send_block send (genesis.hash (), nano::test_genesis_key.pub, nano::genesis_amount - nano::gFLR_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send).code);
...
TEST (ledger, state_account)
{
	...
	auto transaction (store.tx_begin (true));
	store.initialize (transaction, genesis);
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, nano::genesis_account, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
...
TEST (ledger, state_send_receive)
{
	...
	ASSERT_EQ (nano::genesis_amount - nano::gFLR_ratio, ledger.weight (transaction, nano::genesis_account));
...
TEST (ledger, state_receive)
{
	...
	auto transaction (store.tx_begin (true));
	store.initialize (transaction, genesis);
	nano::send_block send1 (genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	ASSERT_TRUE (store.block_exists (transaction, send1.hash ()));
	auto send2 (store.block_get (transaction, send1.hash ()));
	ASSERT_NE (nullptr, send2);
...
TEST (ledger, state_open)
{
	...
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	ASSERT_TRUE (store.block_exists (transaction, send1.hash ()));
	auto send2 (store.block_get (transaction, send1.hash ()));
	ASSERT_NE (nullptr, send2);
	...
	nano::state_block open1 (destination.pub, 0, nano::genesis_account, nano::gFLR_ratio, send1.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, open1).code);
...
TEST (ledger, send_after_state_fail)
{
	...
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, nano::genesis_account, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::send_block send2 (send1.hash (), nano::genesis_account, nano::genesis_amount - (2 * nano::gFLR_ratio), nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
...
TEST (ledger, state_receive_bad_amount_fail)
{
	...
	ASSERT_EQ (nano::gFLR_ratio, ledger.amount (transaction, send1.hash ()));
	ASSERT_EQ (nano::genesis_amount - nano::gFLR_ratio, ledger.weight (transaction, nano::genesis_account));
	nano::state_block receive1 (nano::genesis_account, send1.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, send1.hash (), nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
...
TEST (ledger, state_receive_wrong_account_fail)
{
	...
	ASSERT_EQ (nano::genesis_amount - nano::gFLR_ratio, ledger.weight (transaction, nano::genesis_account));
	nano::keypair key;
	nano::state_block receive1 (key.pub, 0, nano::genesis_account, nano::gFLR_ratio, send1.hash (), key.prv, key.pub, 0);
	ASSERT_EQ (nano::process_result::unreceivable, ledger.process (transaction, receive1).code);
}
...
TEST (ledger, state_state_open_fork)
{
	...
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, open1).code);
	nano::state_block open2 (destination.pub, 0, nano::genesis_account, nano::gFLR_ratio, send1.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::fork, ledger.process (transaction, open2).code);
	ASSERT_EQ (open1.root (), open2.root ());
}
...
TEST (ledger, state_open_previous_fail)
{
	...
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::state_block open1 (destination.pub, destination.pub, nano::genesis_account, nano::gFLR_ratio, send1.hash (), destination.prv, destination.pub, 0);
...
TEST (ledger, state_send_change)
{
	...
	store.initialize (transaction, genesis);
	nano::keypair rep;
	nano::state_block send1 (nano::genesis_account, genesis.hash (), rep.pub, nano::genesis_amount - nano::gFLR_ratio, nano::genesis_account, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	ASSERT_TRUE (store.block_exists (transaction, send1.hash ()));
	auto send2 (store.block_get (transaction, send1.hash ()));
	ASSERT_NE (nullptr, send2);
	...
	ASSERT_EQ (nano::gFLR_ratio, ledger.amount (transaction, send1.hash ()));
	ASSERT_EQ (0, ledger.weight (transaction, nano::genesis_account));
	ASSERT_EQ (nano::genesis_amount - nano::gFLR_ratio, ledger.weight (transaction, rep.pub));
}
...
TEST (ledger, state_receive_old)
{
	...
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::state_block send2 (nano::genesis_account, send1.hash (), nano::genesis_account, nano::genesis_amount - (2 * nano::gFLR_ratio), destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send2).code);
	nano::open_block open1 (send1.hash (), nano::genesis_account, destination.pub, destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, open1).code);
...
TEST (ledger, state_rollback_received_send)
{
	...
	store.initialize (transaction, genesis);
	nano::keypair key;
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::state_block receive1 (key.pub, 0, key.pub, nano::gFLR_ratio, send1.hash (), key.prv, key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, receive1).code);
	ASSERT_FALSE (store.pending_exists (transaction, nano::pending_key (nano::genesis_account, receive1.hash ())));
	ledger.rollback (transaction, send1.hash ());
	ASSERT_FALSE (store.pending_exists (transaction, nano::pending_key (nano::genesis_account, send1.hash ())));
...
TEST (ledger, epoch_blocks_general)
{
	...
	ASSERT_EQ (nano::process_result::block_position, ledger.process (transaction, change1).code);
	nano::state_block send1 (nano::genesis_account, epoch1.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::open_block open1 (send1.hash (), nano::genesis_account, destination.pub, destination.prv, destination.pub, 0);
	...
	ASSERT_EQ (nano::process_result::block_position, ledger.process (transaction, receive1).code);
	nano::state_block receive2 (destination.pub, epoch4.hash (), destination.pub, nano::gFLR_ratio, send1.hash (), destination.prv, destination.pub, 0);
	...
	ASSERT_EQ (nano::genesis_amount - nano::gFLR_ratio, ledger.weight (transaction, nano::genesis_account));
	ASSERT_EQ (nano::gFLR_ratio, ledger.weight (transaction, destination.pub));
}
...
TEST (ledger, epoch_blocks_receive_upgrade)
{
	...
	nano::state_block send1 (nano::genesis_account, genesis.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send1).code);
	nano::state_block epoch1 (nano::genesis_account, send1.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio, 123, epoch_key.prv, epoch_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, epoch1).code);
	nano::state_block send2 (nano::genesis_account, epoch1.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio * 2, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send2).code);
	nano::open_block open1 (send1.hash (), destination.pub, destination.pub, destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, open1).code);
	nano::receive_block receive1 (open1.hash (), send2.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::unreceivable, ledger.process (transaction, receive1).code);
	nano::state_block receive2 (destination.pub, open1.hash (), destination.pub, nano::gFLR_ratio * 2, send2.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, receive2).code);
	...
	nano::state_block send3 (destination.pub, receive2.hash (), destination.pub, nano::gFLR_ratio, destination2.pub, destination.prv, destination.pub, 0);
...
TEST (ledger, unchecked_epoch)
{
	...
	auto epoch1 (std::make_shared<nano::state_block> (destination.pub, open1->hash (), destination.pub, nano::gFLR_ratio, node1.ledger.epoch_link, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0));
...
TEST (ledger, unchecked_epoch_invalid)
{
	...
	auto epoch1 (std::make_shared<nano::state_block> (destination.pub, open1->hash (), destination.pub, nano::gFLR_ratio, node1.ledger.epoch_link, destination.prv, destination.pub, 0));
	node1.work_generate_blocking (*epoch1);
	// Pseudo epoch block (send subtype, destination - epoch link)
	auto epoch2 (std::make_shared<nano::state_block> (destination.pub, open1->hash (), destination.pub, nano::gFLR_ratio - 1, node1.ledger.epoch_link, destination.prv, destination.pub, 0));
