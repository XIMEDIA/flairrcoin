...
TEST (confirmation_height, gap_bootstrap)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		auto send3 (std::make_shared<nano::state_block> (nano::genesis_account, send2->hash (), nano::genesis_account, nano::genesis_amount - 3 * nano::gFLR_ratio, destination.pub, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, 0));
...
TEST (confirmation_height, all_block_types)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		auto & store = node->store;
		// gFLR_ratio changed from Gxrb_ratio
		nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (latest));
		nano::send_block send1 (send.hash (), key2.pub, nano::genesis_amount - nano::gFLR_ratio * 2, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send.hash ()));

		nano::open_block open (send.hash (), nano::dev_genesis_key.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub));
		nano::state_block state_open (key2.pub, 0, 0, nano::gFLR_ratio, send1.hash (), key2.prv, key2.pub, *system.work.generate (key2.pub));

		nano::send_block send2 (open.hash (), key2.pub, 0, key1.prv, key1.pub, *system.work.generate (open.hash ()));
		nano::state_block state_receive (key2.pub, state_open.hash (), 0, nano::gFLR_ratio * 2, send2.hash (), key2.prv, key2.pub, *system.work.generate (state_open.hash ()));

		nano::state_block state_send (key2.pub, state_receive.hash (), 0, nano::gFLR_ratio, key1.pub, key2.prv, key2.pub, *system.work.generate (state_receive.hash ()));
		...
		nano::state_block state_change (key2.pub, state_send.hash (), nano::dev_genesis_key.pub, nano::gFLR_ratio, 0, key2.prv, key2.pub, *system.work.generate (state_send.hash ()));

		nano::state_block epoch (key2.pub, state_change.hash (), nano::dev_genesis_key.pub, nano::gFLR_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (state_change.hash ()));

		nano::state_block epoch1 (key1.pub, change.hash (), key2.pub, nano::gFLR_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (change.hash ()));
		nano::state_block state_send1 (key1.pub, epoch1.hash (), 0, nano::gFLR_ratio - 1, key2.pub, key1.prv, key1.pub, *system.work.generate (epoch1.hash ()));
		nano::state_block state_receive2 (key2.pub, epoch.hash (), 0, nano::gFLR_ratio + 1, state_send1.hash (), key2.prv, key2.pub, *system.work.generate (epoch.hash ()));

		auto state_send2 = std::make_shared<nano::state_block> (key2.pub, state_receive2.hash (), 0, nano::gFLR_ratio, key1.pub, key2.prv, key2.pub, *system.work.generate (state_receive2.hash ()));
		nano::state_block state_send3 (key2.pub, state_send2->hash (), 0, nano::gFLR_ratio - 1, key1.pub, key2.prv, key2.pub, *system.work.generate (state_send2->hash ()));

		nano::state_block state_send4 (key1.pub, state_send1.hash (), 0, nano::gFLR_ratio - 2, nano::dev_genesis_key.pub, key1.prv, key1.pub, *system.work.generate (state_send1.hash ()));
		nano::state_block state_receive3 (nano::genesis_account, send1.hash (), nano::genesis_account, nano::genesis_amount - nano::gFLR_ratio * 2 + 1, state_send4.hash (), nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send1.hash ()));
...
TEST (confirmation_heightDeathTest, rollback_added_block)
{
	...
	if (!nano::running_within_valgrind ())
	{
		...
		nano::keypair key1;
		// gFLR_ratio changed from Gxrb_ratio
		auto send = std::make_shared<nano::send_block> (genesis.hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *pool.generate (genesis.hash ()));
		{
			auto transaction (store.tx_begin_write ());
...
TEST (confirmation_heightDeathTest, modified_chain)
{
	...
	if (!nano::running_within_valgrind ())
	{
		...
		nano::keypair key1;
		// gFLR_ratio changed from Gxrb_ratio
		auto send = std::make_shared<nano::send_block> (nano::genesis_hash, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *pool.generate (nano::genesis_hash));
		{
			auto transaction (store.tx_begin_write ());
			store.initialize (transaction, genesis, ledger.cache);
...
TEST (confirmation_heightDeathTest, modified_chain_account_removed)
{
	...
	if (!nano::running_within_valgrind ())
	{
		...
		// gFLR_ratio changed from Gxrb_ratio
		auto send = std::make_shared<nano::send_block> (nano::genesis_hash, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *pool.generate (nano::genesis_hash));
		auto open = std::make_shared<nano::state_block> (key1.pub, 0, 0, nano::gFLR_ratio, send->hash (), key1.prv, key1.pub, *pool.generate (key1.pub));
		{
			auto transaction (store.tx_begin_write ());
			store.initialize (transaction, genesis, ledger.cache);
			ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, *send).code);
...
TEST (confirmation_height, pending_observer_callbacks)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		// gFLR_ratio changed from Gxrb_ratio
		nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (latest));
		auto send1 = std::make_shared<nano::send_block> (send.hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio * 2, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send.hash ()));
...
TEST (confirmation_height, callback_confirmed_history)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		nano::keypair key1;
		// gFLR_ratio changed from Gxrb_ratio
		auto send = std::make_shared<nano::send_block> (latest, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (latest));
		{
			auto transaction = node->store.tx_begin_write ();
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send).code);
		}
...
TEST (confirmation_height, dependent_election)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		auto send = std::make_shared<nano::send_block> (latest, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (latest));
		auto send1 = std::make_shared<nano::send_block> (send->hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio * 2, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send->hash ()));
		auto send2 = std::make_shared<nano::send_block> (send1->hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio * 3, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send1->hash ()));
		{
			auto transaction = node->store.tx_begin_write ();
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send).code);
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send1).code);
...
TEST (confirmation_height, cemented_gap_below_receive)
{
	auto test_mode = [](nano::confirmation_height_mode mode_a) {
		...
		// gFLR_ratio changed from Gxrb_ratio
		nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (latest));
		nano::send_block send1 (send.hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio * 2, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send.hash ()));
		nano::keypair dummy_key;
		nano::send_block dummy_send (send1.hash (), dummy_key.pub, nano::genesis_amount - nano::gFLR_ratio * 3, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send1.hash ()));
		...
		nano::send_block send2 (receive1.hash (), nano::dev_genesis_key.pub, nano::gFLR_ratio, key1.prv, key1.pub, *system.work.generate (receive1.hash ()));
		...
		nano::send_block dummy_send1 (receive2.hash (), dummy_key.pub, nano::genesis_amount - nano::gFLR_ratio * 3, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (receive2.hash ()));

		nano::keypair key2;
		system.wallet (0)->insert_adhoc (key2.prv);
		nano::send_block send3 (dummy_send1.hash (), key2.pub, nano::genesis_amount - nano::gFLR_ratio * 4, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (dummy_send1.hash ()));
		nano::send_block dummy_send2 (send3.hash (), dummy_key.pub, nano::genesis_amount - nano::gFLR_ratio * 5, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (send3.hash ()));
...
TEST (confirmation_height, election_winner_details_clearing_node_process_confirmed)
{
	...
	// gFLR_ratio changed from Gxrb_ratio
	auto send = std::make_shared<nano::send_block> (nano::genesis_hash, nano::dev_genesis_key.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *system.work.generate (nano::genesis_hash));
...
TEST (confirmation_height, unbounded_block_cache_iteration)
{
	...
	// gFLR_ratio changed from Gxrb_ratio
	auto send = std::make_shared<nano::send_block> (genesis.hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *pool.generate (genesis.hash ()));
	auto send1 = std::make_shared<nano::send_block> (send->hash (), key1.pub, nano::genesis_amount - nano::gFLR_ratio * 2, nano::dev_genesis_key.prv, nano::dev_genesis_key.pub, *pool.generate (send->hash ()));
	{
		auto transaction (store.tx_begin_write ());
		store.initialize (transaction, genesis, ledger.cache);
		ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, *send).code);
